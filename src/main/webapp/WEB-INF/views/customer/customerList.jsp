						...
						<div class="panel panel-default"
							...
							<div class="panel-heading">
								...
								<ul class="panel-controls">
									<li><a href="#" class="panel-refresh"
										onclick="location.reload();"><span class="fa fa-refresh"
											style="margin-top: 26%;"></span></a></li>
								</ul>
							</div>
											...
											<div class="row">
												<div class="col-md-2">
													<div class="form-group">
														<label>Customer Type</label> <select name="userStatus"
															Class="form-control select">
															<option value="U"
																<c:if test="${userStatus=='U'}">selected</c:if>>-All-</option>
															<option value="N"
																<c:if test="${userStatus=='N'}">selected</c:if>>New</option>
															<option value="A"
																<c:if test="${userStatus=='A'}">selected</c:if>>Active</option>
															<option value="D"
																<c:if test="${userStatus=='D'}">selected</c:if>>Inactive</option>
														</select>
													</div>
												</div>
										...
										<table class="table datatable table-bordered">
											...
											<tbody>
												<c:forEach items="${userList}" var="userItem">
													<tr id="user" data-user-id="${userItem.userId}"
														class="clickableRow <c:if test="${userItem.userStatus eq 'D'.charAt(0)}" >danger</c:if> <c:if test="${userItem.userStatus eq 'A'.charAt(0)}" >success</c:if>">
														...
														<td><fmt:formatDate value="${userItem.dob}"
