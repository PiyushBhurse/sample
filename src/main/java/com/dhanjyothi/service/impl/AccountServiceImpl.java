...
public class AccountServiceImpl implements AccountService {
	...
	public List<Account> getAccountDetails(long userId, String accountType) throws Exception {
		...
		return accountListByUserId.stream().distinct().filter(a -> a.getAcctType() == accountType.charAt(0))
	...
	public int checkAccountExists(long benAcctId) throws Exception {
		...
		try {
			...
			return this.accountDao.existsByAcctIdAndAcctType(benAcctId, 'S') ? 3 : 4;// 3:account exists|4:account not
		} catch (Exception e) {
			return 5;// user dont have account
		}
	...
	public List<Transaction> loadTransactionsBetweenStandEnDt(String startDate, String endDate) throws Exception {
		...
		return transactions.stream().distinct()// get unique values
				.filter(t -> (t.getTranDtTime().after(startDt) && t.getTranDtTime().before(endDt)))// get data within st
																									// and ed date range
	...
	public List<User> getUserList(char userStatus) {
		...
		List<User> filteredList = allUserList.stream().filter(u -> u.getUserStatus() == userStatus)
				.collect(Collectors.toList());
	...
	public List<Beneficiaries> getAllBeneficiariesForAccount() {
		try {
			User user = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal())
					.getUser();
			Account savingAccount = this.getAccountDetails(user.getUserId(), "S").get(0);
			List<Beneficiaries> beneficiaryListforAccount = this.beneficiaryDao
					.getBeneficiaryListforAccount(savingAccount.getAcctId());
			return beneficiaryListforAccount.stream().distinct().collect(Collectors.toList());
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public void transferFundsToBeneficiary(Transaction transaction) {
		try {
			String tranDesc = transaction.getTranDesc();
			Date currentDate = DhanJyothiUtil.getCurrentDate();
			User user = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal())
					.getUser();
			Account savingAccount = this.getAccountDetails(user.getUserId(), "S").get(0);
			...
			transaction.setTranDesc(transaction.getTranDesc() + "; transfered to account#"
					+ transaction.getBeneficiaries().getBenAccNum());
			...
			beneficiaryAccountTransaction
					.setTranDesc(tranDesc + "; transfered by account#" + savingAccount.getAcctId());
	...
	public void saveServiceRequest(ServiceRequest serviceRequest) {
		try {
			...
			int chequeRequestCharges = getChequeBookPrice(serviceRequest.getReqDesc());
			System.out.println("price>>>"+chequeRequestCharges);
			Date currentDate = DhanJyothiUtil.getCurrentDate();
			User user = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal())
					.getUser();
			Account savingAccount = this.getAccountDetails(user.getUserId(), "S").get(0);
	...
	public int getChequeBookPrice(String pageDesc) {
