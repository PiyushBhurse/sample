...
public class AccountController {
	...
	public String loadAccountCreationPage(Model model, HttpServletRequest request) {
		...
		User curUser = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal())
				.getUser();
		try {
	...
	public String accountHome(ModelMap model, HttpServletRequest request) throws Exception {
		...
		model.addAttribute("savingAccount",
				(savingAccount != null && !savingAccount.isEmpty()) ? savingAccount.get(0) : new Account());
		...
		model.addAttribute("chequeReqList", DhanJyothiUtil.getChequeRequestList());
	...
	public String showBeneficiariesView(ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<Account> savingAccount = this.accountService.getAccountDetails(user.getUserId(), "S");
		model.addAttribute("savingAccount",
				(savingAccount != null && !savingAccount.isEmpty()) ? savingAccount.get(0) : null);
		model.addAttribute("account", new Account());
		model.addAttribute("tenure", DhanJyothiUtil.getTenureDetails());
		model.addAttribute("startDate", startDate);
		model.addAttribute("endDate", endDate);
	...
	public String addBeneficiary(@ModelAttribute("beneficiary") Beneficiaries beneficiaries,
	...
	public String transferFundsToAccount(@ModelAttribute("fundTransaction") Transaction transaction,
			HttpServletRequest request) {
	...
	public String addServiceRequest(HttpServletRequest request) {
		...
		ServiceRequest sr = new ServiceRequest(reqDesc);
