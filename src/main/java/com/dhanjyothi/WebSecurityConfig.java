...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/css/**", "/js/**", "/imgs/**").permitAll()
				...
				.antMatchers("/account/sendServiceRequest**").hasAnyAuthority("C")
				.antMatchers("/account/fundTransfer**").hasAnyAuthority("C").anyRequest().denyAll().and()
				.exceptionHandling().accessDeniedPage("/errors/accessDenied").and().formLogin().loginPage("/home/login")
				.defaultSuccessUrl("/home/index", true).permitAll().and().logout().permitAll()
				.logoutSuccessUrl("/home/login?logout").and().sessionManagement().maximumSessions(1)
				.expiredUrl("/home/login?expired");
	}
